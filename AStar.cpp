...
vector<Point> Dijkstras(Point start, Point goal, int obstacles[env_x][env_y][env_z], int min_x, int max_x, int min_y, int max_y, int min_z, int max_z) {
  ...
  front.push({.00001+distanceBetweenPoints(start, goal), .00001, start, NULL});
  ...
  while(size(front)>0){
    ...
    */
    ...
    for(int i = 0, i <= 6, i++){
      ...
      new_pos.y = new_y;
      new_pos.z = new_z;
      if(visited[new_pos.x][new_pos.y][new_pos.z]==0 && obstacles_in_env[env_x][env_y][env_z] ==0){
        //new_cost is the cost from start to the new_pos plus the cost of the obstacles
        //obstacle cost is the cost of the obstacle at our new_pos/64
        //64 determines the tradeoff between avoiding obstacles and going further
        //new_total_cost is the new_cost plus the estimated cost from new_pos to goal
        int new_cost = cost_of_node+movements[i].cost+obstacles_in_env[env_x][env_y][env_z]/64;
        front.push(new_cost+distance(new_pos, goal), new_cost, new_pos, pos_of_node)
      }
    }
  }
...
int main(){
  ...
  results = Dijkstras(start, goal, obstacles_in_env, min_x, max_x, min_y, max_y, min_z, max_z);
